...
from app.projects.jkn.api.deps import get_auth_service, get_current_user
...
from app.projects.jkn.infra.token import create_token
...
async def login(
    ...
    try:
        ...
        refresh_token = await service.create_refresh_token(
            user_id, email, payload.device_id
        )
        return TokenResponse(
            access_token=create_token(user_id, email), refresh_token=refresh_token
        )
...
async def google_login(
    ...
    try:
        ...
        refresh_token = await service.create_refresh_token(
            google_id, email, payload.device_id
        )
        return TokenResponse(
            access_token=create_token(google_id, email), refresh_token=refresh_token
        )
...
async def refresh_token(
    ...
    try:
        user_id, email, new_refresh_token = await service.rotate_refresh_token(
            payload.refresh_token, payload.device_id
        )
        return TokenResponse(
            access_token=create_token(user_id, email), refresh_token=new_refresh_token
        )
