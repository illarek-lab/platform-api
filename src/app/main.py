...
CREDENTIALS_FMC_DIR = Path(__file__).resolve().parents[2] / "credentials_FMC"
CREDENTIALFMC_DIR_CREATED = CREDENTIALS_FMC_DIR.mkdir(exist_ok=True)
...
_fcm_students_col = _mongo_db["mobile_student_fcm_endpoint"]
...
<div class="topbar">
  ...
  <div class="topbar-links">
    ...
    <a href="/credentials_FMC/ui">Subir Credentials FCM</a>
    <a href="/docs">Swagger UI</a>
...
def _safe_fcm_filename(name: str) -> str | None:
    if not name or ".." in name or "/" in name or "\\" in name:
        return None
    if not name.endswith("-firebase-adminsdk.json"):
        return None
    prefix = name[:-23]
    safe_prefix = re.sub(r"[^a-zA-Z0-9_\-]", "_", prefix)
    if safe_prefix != prefix or not safe_prefix:
        return None
    return name


@app.post("/credentials_FMC/upload", tags=["credentials_FMC"])
async def upload_fcm_credential(file: UploadFile):
    ...
    if not safe:
        return JSONResponse(
            status_code=400,
            content={
                "detail": "Nombre inválido. Debe ser un archivo {project_slug}-firebase-adminsdk.json con solo letras, números, guiones y guiones bajos."
            },
        )
    ...
    try:
        ...
        return JSONResponse(
            status_code=400,
            content={"detail": "El archivo no es un JSON válido."},
        )

    required_keys = {"type", "project_id", "private_key", "client_email"}
    missing_keys = sorted(list(required_keys - data.keys()))
    if missing_keys:
        return JSONResponse(
            status_code=400,
            content={
                "detail": f"Faltan claves requeridas en el JSON de Firebase: {', '.join(missing_keys)}"
            },
        )

    (CREDENTIALS_FMC_DIR / safe).write_bytes(content)

    project_name = safe.removesuffix("-firebase-adminsdk.json")
    await _fcm_students_col.update_one(
        {"project_name": project_name},
        {
            "$set": {
                "project_name": project_name,
...
@app.get("/credentials_FMC/students", tags=["credentials_FMC"])
...
async def credentials_fcm_ui():
    ...
    if not students:
        file_rows = '<tr><td colspan="2" class="empty">No hay credenciales FCM registradas</td></tr>'
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Subir Credentials FCM — Illarek Lab</title>
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{ font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; background: #f0f2f5; color: #1a202c; min-height: 100vh; }}
  ...
  .topbar {{ background: #fff; border-bottom: 1px solid #e2e8f0; padding: 0 40px; display: flex; align-items: center; justify-content: space-between; height: 62px; box-shadow: 0 1px 4px rgba(0,0,0,.04); }}
  .topbar-brand {{ display: flex; align-items: center; gap: 10px; text-decoration: none; }}
  .brand-logo {{ width: 30px; height: 30px; background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); border-radius: 8px; display: flex; align-items: center; justify-content: center; }}
  .brand-logo svg {{ width: 16px; height: 16px; fill: #fff; }}
  .brand-text {{ display: flex; flex-direction: column; line-height: 1; }}
  .brand-name {{ font-size: 0.92rem; font-weight: 800; color: #0f172a; letter-spacing: -.02em; }}
  .brand-sub {{ font-size: 0.65rem; font-weight: 500; color: #94a3b8; letter-spacing: .04em; text-transform: uppercase; margin-top: 1px; }}
  .topbar-links {{ display: flex; gap: 8px; }}
  .topbar-links a {{ font-size: 0.78rem; font-weight: 600; color: #d97706; text-decoration: none; border: 1px solid #fef3c7; background: #fffbeb; border-radius: 7px; padding: 5px 14px; transition: background .15s; }}
  .topbar-links a:hover {{ background: #fef3c7; }}
  ...
  .upload-zone svg {{ width: 48px; height: 48px; stroke: #94a3b8; margin-bottom: 12px; }}
  .upload-zone p {{ color: #64748b; font-size: 0.88rem; margin-bottom: 16px; }}
  .upload-zone .btn-upload {{ display: inline-flex; align-items: center; gap: 8px; background: linear-gradient(135deg, #f59e0b, #d97706); color: #fff; font-size: 0.88rem; font-weight: 700; border: none; border-radius: 8px; padding: 10px 24px; cursor: pointer; transition: transform .1s, box-shadow .2s; }}
  .upload-zone .btn-upload:hover {{ transform: translateY(-1px); box-shadow: 0 4px 12px rgba(245,158,11,.3); }}
  .upload-zone input[type="file"] {{ display: none; }}
...
<div class="container">
  <h1>Subir Credentials FCM</h1>
  <p class="subtitle">Sube archivos JSON de Service Account (ej. <code>layout_example-firebase-adminsdk.json</code>) al directorio <code>credentials_FMC/</code> del proyecto.</p>
  ...
  <div class="upload-zone" id="dropZone">
    ...
    <p>Arrastra un archivo <strong>-firebase-adminsdk.json</strong> aquí o haz clic en el botón</p>
    <button class="btn-upload" onclick="document.getElementById('fileInput').click()">
      <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
      Subir Credentials FCM
    </button>
  ...
  <div class="files-card">
    <div class="files-header">Credenciales FCM registradas</div>
    <table id="filesTable">
      {file_rows}
    </table>
  </div>
...
<footer>
  <strong>illarek-lab</strong> &mdash; Platform API &middot; FCM Credentials Manager
</footer>
