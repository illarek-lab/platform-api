...
CREDENTIALS_FMC_DIR.mkdir(exist_ok=True)
FCM_FILENAME_SUFFIX = "-firebase-adminsdk.json"
REQUIRED_FCM_KEYS = {"type", "project_id", "private_key", "client_email"}
...
def _safe_fcm_filename(name: str) -> str | None:
    if not name or ".." in name or "/" in name or "\\" in name:
        return None
    safe = re.sub(r"[^a-zA-Z0-9_\-.]", "_", name)
    if safe != name or not safe.endswith(FCM_FILENAME_SUFFIX):
        return None
    return safe
...
async def upload_fcm_credential(file: UploadFile):
    ...
    if not safe:
        return JSONResponse(
            status_code=400,
            content={
                "detail": f"Nombre inválido. Debe ser un archivo '{{project_slug}}{FCM_FILENAME_SUFFIX}' con solo letras, números, guiones y guiones bajos."
            },
        )
    ...
    missing = sorted(REQUIRED_FCM_KEYS - payload.keys())
    empty = sorted(k for k in REQUIRED_FCM_KEYS & payload.keys() if not payload[k])
    ...
    project_name = safe.removesuffix(FCM_FILENAME_SUFFIX)
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Subir Firebase Credentials — Illarek Lab</title>
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{ font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; background: #f0f2f5; color: #1a202c; min-height: 100vh; }}
...
<div class="topbar">
  ...
  <div class="topbar-links">
    <a href="/">Dashboard</a>
    <a href="/credentials/ui">Subir Credentials (.env)</a>
    <a href="/docs">Swagger UI</a>
  </div>
</div>

<div class="container">
  <h1>Subir Firebase Credentials</h1>
  <p class="subtitle">Sube el JSON de la cuenta de servicio (<code>serviceAccountKey.json</code>) al directorio <code>credentials_FMC/</code> del proyecto. El archivo debe llamarse <code>{{project_slug}}{FCM_FILENAME_SUFFIX}</code>.</p>
  ...
  <div class="upload-zone" id="dropZone">
    ...
    <p>Arrastra el <strong>{FCM_FILENAME_SUFFIX}</strong> aquí o haz clic en el botón</p>
    <button class="btn-upload" onclick="document.getElementById('fileInput').click()">
      <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
      Subir Firebase JSON
    </button>
...
<footer>
  <strong>illarek-lab</strong> &mdash; Platform API &middot; Firebase Credentials Manager
</footer>
